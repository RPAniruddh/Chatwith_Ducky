...
st.set_page_config(page_title="Ducky")
...
if 'bot_response' not in st.session_state:
    st.session_state['bot_response'] = ["I'm Ducky, what can I help you with ?"]
...
def get_input():
    input_text = st.text_input("come on ask me: ", "", key="input")
    return input_text
...
with response_container:
    ...
    if st.session_state['bot_response']:
        for i in range(len(st.session_state['bot_response'])):
            ...
            message(str(st.session_state['bot_response'][i]), key=str(i))